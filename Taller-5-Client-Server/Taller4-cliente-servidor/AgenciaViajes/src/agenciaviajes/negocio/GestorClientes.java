...
public class GestorClientes extends AModel {
    ...
    private ConectorJdbcServidorCentral conector2;
    
    ArrayList<Cliente> atrClientes = new ArrayList();

    public GestorClientes() {
        registraduria = new ServicioRegistraduriaSocket();
        conector = new ConectorJdbc();
        conector2 = new ConectorJdbcServidorCentral();
    }

    //Obtiene el numero de Hombres;
    public int numeroHombres()throws SQLException, ClassNotFoundException{
        conector.conectarse();
        conector.crearConsulta("SELECT COUNT(ID) AS cantidad FROM clientes WHERE sexo='Masculino';");
        int numHombres = 0;
        if(conector.getResultado().next()){
            String auxiliar = conector.getResultado().getString("cantidad");
            numHombres = Integer.parseInt(auxiliar);
        }
        return numHombres;
    }
    
    //Obtiene el numero de Mujeres;
    public int numeroMujeres()throws ClassNotFoundException, SQLException{
        conector.conectarse();
        conector.crearConsulta("SELECT COUNT(ID) AS cantidad FROM clientes WHERE sexo='Femenino';");
        int numMujeres = 0;
        if(conector.getResultado().next()){
            String auxiliar = conector.getResultado().getString("cantidad");
            numMujeres = Integer.parseInt(auxiliar);
        }
        return numMujeres;
    }
    ...
    public void agregarCliente(String id, String nombres, String apellidos, String direccion, String celular, String email, String sexo) throws ClassNotFoundException, SQLException {
        ...
        conector2.conectarse();
        conector2.actualizar("INSERT INTO Clientes (id, nombres, apellidos, direccion, celular, email, sexo)"
                + " VALUES ("
                + "'" + id + "',"
                + "'" + nombres + "',"
                + "'" + apellidos + "',"
